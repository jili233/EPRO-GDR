...
osmesaprefix="${OSMESA_PREFIX:-/home/gdrnpp/bop_renderer/osmesa}"

# Location of the LLVM installation:
llvmprefix="${LLVM_PREFIX:-/home/gdrnpp/bop_renderer/llvm}"
...
if [ "$osmesadriver" = 3 ] || [ "$osmesadriver" = 4 ]; then
    ...
    if [ "$buildllvm" = 1 ]; then
        ...
        $xzcat llvm-${llvmversion}.src.tar.$archsuffix | tar --no-same-owner -xf -
...
tar --no-same-owner -zxf "mesa-${mesaversion}.tar.gz"
...
tar --no-same-owner -jxf glu-${gluversion}.tar.bz2
cd glu-${gluversion}
echo "* building GLU..."
confopts="\
    --disable-dependency-tracking \
...
tar --no-same-owner -jxf mesa-demos-${demoversion}.tar.bz2
